...
- **OSM sidecar PostGIS selection is an intentional, exact-match coupling.** `provisionPostgresImage()` in `src/lib/mcp-server.ts` swaps in `postgis/postgis:<pg>-3.5` only when the database name is `osm_lead_source` **and** the env-var key is `OSM_LEAD_SOURCE_PRODUCTION_DATABASE_URL`. Every other provisioning request stays on `postgres:<pg>`. The pairing is pinned by `src/__tests__/provision-postgres-image.test.ts`; see `docs/osm-sidecar-postgis.md` before changing or widening it. The `fix/osm-postgis-provisioning` branch is superseded by `main` and must not be merged.
